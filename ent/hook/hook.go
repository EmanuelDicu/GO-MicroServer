...
// The TemperatureFunc type is an adapter to allow the use of ordinary
// function as Temperature mutator.
type TemperatureFunc func(context.Context, *ent.TemperatureMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TemperatureFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TemperatureMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TemperatureMutation", m)
}
