...
func (cq *CountryQuery) loadCities(ctx context.Context, query *CityQuery, nodes []*Country, init func(*Country), assign func(*Country, *City)) error {
	...
	for _, n := range neighbors {
		fk := n.id_tara
		if fk == nil {
			return fmt.Errorf(`foreign-key "id_tara" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "id_tara" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
