...
type CityQuery struct {
	...
	withTemperatures *TemperatureQuery
	withFKs          bool
...
// QueryTemperatures chains the current query on the "Temperatures" edge.
func (cq *CityQuery) QueryTemperatures() *TemperatureQuery {
	query := (&TemperatureClient{config: cq.config}).Query()
...
func (cq *CityQuery) Clone() *CityQuery {
	...
	return &CityQuery{
		...
		withTemperatures: cq.withTemperatures.Clone(),
...
// WithTemperatures tells the query-builder to eager-load the nodes that are connected to
// the "Temperatures" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CityQuery) WithTemperatures(opts ...func(*TemperatureQuery)) *CityQuery {
	query := (&TemperatureClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withTemperatures = query
	return cq
}
...
func (cq *CityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*City, error) {
	...
	if query := cq.withTemperatures; query != nil {
		if err := cq.loadTemperatures(ctx, query, nodes,
			func(n *City) { n.Edges.Temperatures = []*Temperature{} },
			func(n *City, e *Temperature) { n.Edges.Temperatures = append(n.Edges.Temperatures, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CityQuery) loadTemperatures(ctx context.Context, query *TemperatureQuery, nodes []*City, init func(*City), assign func(*City, *Temperature)) error {
	...
	query.withFKs = true
	query.Where(predicate.Temperature(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(city.TemperaturesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.id_oras
		if fk == nil {
			return fmt.Errorf(`foreign-key "id_oras" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "id_oras" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
